...
export default function ResumeButton({ participation }: Props) {
  ...
  // Total modul belajar (exclude certificate)
  const totalLearnable = countLearnables(course);

  // Jumlah modul yang selesai (clamp 0..total)
  const completedRaw =
  ...
  const percent =
    totalLearnable === 0
      ? 100
      : Math.round((completed / totalLearnable) * 100);
  ...
  return (
    ...
    >
      ...
      <div className="mt-3">
        ...
        {/* angka kecil optional */}
        <div className="mt-1 text-xs text-white/60">
          {totalLearnable > 0
            ? `${completed}/${totalLearnable} • ${percent}%`
            : "0/0 • 100%"}
        </div>
      </div>
    </Link>
